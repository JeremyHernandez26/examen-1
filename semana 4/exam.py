...
def do_count():
    if val_input.value is not None:
        ...
        ui.notify(
            f'El valor {val_input.value} aparece {cantidad} vez/veces.',
            color='positive'
        )
    else:
        ui.notify('Error: Introduce un valor para usar count().', color='warning')
...
def do_reverse():
    item.reverse()
    render_list()
    ui.notify('La lista ha sido invertida.', color='positive')


# Metodo remove
def do_remove():
    if val_input.value is not None:
        if val_input.value in item:
            item.remove(val_input.value)
            render_list()
            ui.notify(
                f'El valor {val_input.value} fue eliminado.',
                color='positive'
            )
        else:
            ui.notify(
                f'El valor {val_input.value} no existe en la lista.',
                color='warning'
            )
    else:
        ui.notify('Error: Introduce un valor para usar remove().', color='warning')
...
def do_sort():
    item.sort()
    render_list()
    ui.notify('La lista ha sido ordenada de menor a mayor.', color='positive')
...
with ui.column().classes('p-6 gap-6 w-full'):
    ...
    with ui.card().classes('w-full p-4 gap-4'):
        ...
        with ui.row():
            ...
            ui.button('count(x)', icon='123', color='purple', on_click=do_count)
            ui.button('reverse()', icon='swap_vert', color='blue', on_click=do_reverse)
            ui.button('remove(x)', icon='delete_forever', color='orange', on_click=do_remove)
